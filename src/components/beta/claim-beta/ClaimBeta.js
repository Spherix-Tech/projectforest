...
const ClaimBeta = () => {
  ...
  const redirectToSignup = () => {
    setCookies("signup", "beta");
    router.push("/signup");
  };

  return (
    <div
      className="py-10 text-center flex flex-col items-center gap-5 relative  w-full"
      id="claim"
    >
      ...
      <p className="md:w-[60%] w-full text-labelTextColor text-opacity-80 md:text-lg text-sm ">
        Complete steps 1 & 2 below to and get a chance to receive an official
        invitation to Project Forest&apos;s closed beta on the 27th of
        September. Register now, there are only 1000 slots available for the
      ...
      <button
        ...
        className="flex md:gap-4 gap-2 justify-center items-center w-full md:w-3/4 md:py-6 py-5 btnBeta"
      >
