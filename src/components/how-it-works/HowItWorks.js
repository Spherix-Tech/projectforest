...
const HowItWorks = ({ howItWorksItemsList, haveRoundedPoints = false }) => {
  ...
  useEffect(() => {
    if (inView) {
      ...
      for (let i = 0; i < howItWorksItemsList.length; i++) {
        t1.to(`#card${i}`, { opacity: 1, x: 0, delay: 0.05 * i });
      }
    }
      ...
      >
        ...
        <div className="hidden lg:block">
          <div className="w-full flex flex-col lg:flex-row justify-between items-center gap-[1rem] md:gap-[8rem]">
            ...
            <div
              ref={ref}
              className="w-full xl:flex-1 relative overflow-y-scroll md:overflow-hidden max-h-[350px] md:max-h-[none] md:py-[2rem] xl:py-[1rem]"
            >
